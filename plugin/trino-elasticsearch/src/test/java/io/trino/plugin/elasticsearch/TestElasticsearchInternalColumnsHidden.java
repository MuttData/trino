...
public class TestElasticsearchInternalColumnsHidden
        extends AbstractTestQueryFramework
{
    ...
    @Test
    public void testInternalColumnsNotVisible()
    {
        // Verify that internal columns are not visible in metadata when using default config
        assertThat(computeActual("SELECT column_name FROM information_schema.columns WHERE table_schema = 'tpch' AND table_name = 'nation'")
                .getOnlyColumnAsSet())
                .doesNotContain("_id", "_source", "_score");
        
        // Verify we can still query them even though they're hidden
